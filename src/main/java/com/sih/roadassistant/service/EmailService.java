...
public class EmailService {
    ...
    @Async
    public void sendResetPasswordEmail(String recipientEmail, String code) {
        try {
            ...
            helper.setFrom(senderEmail, "Road Assistant");
            helper.setTo(recipientEmail);
            helper.setSubject("Road Assistant - Reset Your Password");

            String htmlContent = "<html>"
                    + "<body style='font-family: Arial, sans-serif; background-color: #f4f7f6; padding: 20px;'>"
                    + "<div style='max-width: 600px; margin: auto; background: #ffffff; padding: 40px; border-radius: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); border-top: 5px solid #2F3E46;'>"
                    + "<h2 style='color: #2F3E46; text-align: center; margin-bottom: 20px;'>Password Reset Request</h2>"
                    + "<p style='font-size: 16px; color: #354F52; line-height: 1.5;'>We received a request to reset your RoadSathi account password. Please use the following code to complete your request:</p>"
                    + "<div style='background-color: #CAD2C5; padding: 15px; border-radius: 6px; text-align: center; margin: 30px 0;'>"
                    + "<span style='font-size: 32px; font-weight: bold; letter-spacing: 6px; color: #2F3E46;'>" + code + "</span>"
                    + "</div>"
                    + "<p style='font-size: 13px; color: #84A98C; text-align: center;'>This password reset code is temporary and will expire in 15 minutes.</p>"
                    + "<hr style='border: none; border-top: 1px solid #ECEEEF; margin: 30px 0;'>"
                    + "<p style='font-size: 11px; color: #999; text-align: center;'>If you did not request a password reset, you can safely ignore this email.</p>"
                    + "</div>"
                    + "</body>"
                    + "</html>";
